...
const DEFAULT_OPENAI_API_URL = 'https://api.openai.com';
const OPENAI_CHAT_COMPLETION_CALL_SITE = 'openai-api-provider-harness:sendChatCompletion';
const OPENAI_SOURCE_FILE = 'server/src/services/providers/openai-api-provider-harness.js';
...
function emitProviderEvent(onProviderEvent, type, data = {}) {
  ...
  try {
    onProviderEvent(type, {
      providerId: 'openai',
      providerHarness: 'openai-api',
      timestamp: nowIso(),
      ...data,
    });
  } catch (err) {
    console.warn('[openai-api-provider-harness] provider event listener failed:', err.message);
  }
}
...
function queueCapture({
  ...
  const captureInput = {
    ...
    providerResearchId: 'openai-api',
    providerPathType: 'direct-http',
    callSite: captureContext.callSite || OPENAI_CHAT_COMPLETION_CALL_SITE,
    ...
    outcome,
    source: {
      file: OPENAI_SOURCE_FILE,
      functionName: captureContext.functionName || 'sendOpenAiChatCompletion',
...
function sendJsonRequest({
  ...
  emitUserVisibleProviderStatus(
    ...
    'openai agent sent payload to openai',
    'sent',
    {
      callSite: providerTrace.callSite,
      operation: providerTrace.operation,
  ...
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          emitUserVisibleProviderStatus(
            ...
            'openai agent received payload from openai',
            'received',
            {
              statusCode: finalTrace.statusCode,
              durationMs: finalTrace.durationMs,
          ...
          if (capture.queued) {
            ...
            emitUserVisibleProviderStatus(
              ...
              'openai agent sent payload to database',
              'queued',
              {
                providerPackageId: finalTrace.providerPackageId,
                captureEnabled: true,
    ...
    req.on('timeout', () => {
      const err = new Error(`OpenAI API request timed out after ${timeoutMs}ms`);
...
async function sendOpenAiChatCompletion({
  ...
  timeoutMs,
  getApiKey,
  baseUrl = DEFAULT_OPENAI_API_URL,
  captureContext = {},
  onProviderEvent,
} = {}) {
  const effectiveModel = model || body?.model || captureContext.modelRequested || '';
  const apiKey = typeof getApiKey === 'function' ? await getApiKey('openai') : null;
  if (!apiKey) {
  ...
  const effectiveCaptureContext = {
    ...
    providerResearchId: 'openai-api',
    providerPathType: 'direct-http',
    callSite: OPENAI_CHAT_COMPLETION_CALL_SITE,
    operation: 'chat-completion',
    functionName: 'sendOpenAiChatCompletion',
    modelRequested: effectiveModel || requestBody.model || '',
    ...captureContext,
    metadata: {
      ...(captureContext.metadata || {}),
    },
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    const err = new Error(`OpenAI API error (HTTP ${response.statusCode}): ${(response.body || '').slice(0, 500)}`);
  ...
  if (response.parseError) {
    const err = new Error(`OpenAI returned invalid JSON: ${(response.body || '').slice(0, 200)}`);
    err.code = 'PROVIDER_ERROR';
    providerTrace = withProviderTraceUpdates(providerTrace, {
  ...
  providerTrace = withProviderTraceUpdates(providerTrace, {
    outcome: 'success',
    model: response.parsedJson?.model || effectiveModel || requestBody.model || '',
...
module.exports = {
  OPENAI_CHAT_COMPLETION_CALL_SITE,
