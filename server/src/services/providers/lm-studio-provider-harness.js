...
const DEFAULT_LM_STUDIO_API_URL = process.env.LM_STUDIO_API_URL || 'http://127.0.0.1:1234';
const DEFAULT_LM_STUDIO_API_TOKEN = process.env.LM_STUDIO_API_TOKEN || process.env.LM_STUDIO_API_KEY || null;
const LM_STUDIO_CHAT_COMPLETION_CALL_SITE = 'lm-studio-provider-harness:sendChatCompletion';
const LM_STUDIO_SOURCE_FILE = 'server/src/services/providers/lm-studio-provider-harness.js';
...
function emitProviderEvent(onProviderEvent, type, data = {}) {
  ...
  try {
    onProviderEvent(type, {
      ...
      providerHarness: 'lm-studio-openai-compatible',
      timestamp: nowIso(),
      ...data,
    });
  } catch (err) {
    console.warn('[lm-studio-provider-harness] provider event listener failed:', err.message);
  }
}
...
function queueCapture({
  ...
  const captureInput = {
    ...
    providerResearchId: 'lm-studio-openai-compatible',
    providerPathType: 'lm-studio-http-nonstream',
    callSite: captureContext.callSite || LM_STUDIO_CHAT_COMPLETION_CALL_SITE,
    ...
    outcome,
    source: {
      file: LM_STUDIO_SOURCE_FILE,
      functionName: captureContext.functionName || 'sendLmStudioChatCompletion',
  ...
  const queued = recordLmStudioProviderCallPackageInBackground(captureInput, {
    packageId,
...
function sendJsonRequest({
  ...
  emitUserVisibleProviderStatus(
    ...
    'lm-studio agent sent payload to lm-studio',
    'sent',
    {
      callSite: providerTrace.callSite,
      operation: providerTrace.operation,
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      ...
      (res) => {
        ...
        const responseHeadersAt = nowIso();
        ...
        res.on('end', () => {
          ...
          emitUserVisibleProviderStatus(
            ...
            'lm-studio agent received payload from lm-studio',
            'received',
            {
              statusCode: finalTrace.statusCode,
              durationMs: finalTrace.durationMs,
          ...
          if (capture.queued) {
            ...
            emitUserVisibleProviderStatus(
              ...
              'lm-studio agent sent payload to database',
              'queued',
              {
                providerPackageId: finalTrace.providerPackageId,
                captureEnabled: true,
    ...
    if (signal?.aborted) {
      const err = new Error('LM Studio request aborted');
      err.name = 'AbortError';
      err.code = 'ABORT_ERR';
    ...
    req.on('timeout', () => {
      const err = new Error(`LM Studio request timed out after ${timeoutMs}ms`);
...
async function sendLmStudioChatCompletion({
  body,
  model,
  timeoutMs,
  captureContext = {},
  ...
  const effectiveModel = model || body?.model || 'local';
  const requestBody = {
    ...(body && typeof body === 'object' ? body : {}),
    model: effectiveModel,
  };
  const headers = DEFAULT_LM_STUDIO_API_TOKEN
    ? { Authorization: `Bearer ${DEFAULT_LM_STUDIO_API_TOKEN}` }
    : {};
  const effectiveCaptureContext = {
    providerId: 'lm-studio',
    providerResearchId: 'lm-studio-openai-compatible',
    providerPathType: 'lm-studio-http-nonstream',
    callSite: LM_STUDIO_CHAT_COMPLETION_CALL_SITE,
    operation: 'chat-completion',
    functionName: 'sendLmStudioChatCompletion',
  ...
  try {
    response = await sendJsonRequest({
      method: 'POST',
      baseUrl: DEFAULT_LM_STUDIO_API_URL,
    ...
    const providerTrace = err.providerTrace;
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    const err = new Error(`LM Studio error (HTTP ${response.statusCode}): ${(response.body || '').slice(0, 500)}`);
  ...
  if (response.parseError) {
    const err = new Error(`LM Studio returned invalid JSON: ${(response.body || '').slice(0, 200)}`);
    err.code = 'PROVIDER_ERROR';
    providerTrace = withProviderTraceUpdates(providerTrace, {
...
module.exports = {
  LM_STUDIO_CHAT_COMPLETION_CALL_SITE,
