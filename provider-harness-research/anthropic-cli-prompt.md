...
Local meaning of `anthropic-cli` in this application:

`anthropic-cli` means the application reaches Anthropic through a local command-line or login-backed subscription path, not through the direct Anthropic HTTP API key path. Treat this as a provider path where this app launches or talks to a local Anthropic/Claude command process and receives whatever that process returns.

Do not assume this is the same as `anthropic-api`.

Do not assume this is the same as `claude-code` unless the current app source proves that the app is actually invoking Claude Code for this provider path.

Your job is to document what this app actually does today. If the app currently uses a local Claude command, SDK wrapper, spawned process, stdout/stderr, JSON output mode, or any other CLI-adjacent path, document the exact path and what raw package reaches this server.
...
provider-harness-research/providers/anthropic-cli.md
...
- State clearly whether the current code calls it `anthropic-cli`, `claude`, `claude-code`, `anthropic-sdk`, or something else.
- If `anthropic-cli` is only a research label and the app uses a different actual provider id, state both clearly:
...
- If there is no current implementation for `anthropic-cli`, state that directly and list the closest related code paths.
...
- Include command name, command args, stdin payload, env var names, model, temperature, max token, image payload, streaming, and timeout fields if present.
...
- Use official Anthropic documentation where available.
- If the path is a CLI command, document the documented CLI output modes and error output modes.
- If the path is really Claude Code or another Anthropic product, document that product accurately and say why it applies.
...
- Do not modify anything outside `provider-harness-research/providers/anthropic-cli.md`.
- Before finishing, run `git diff -- provider-harness-research/providers/anthropic-cli.md` and make sure the only changed file is your provider document.
