...
- **Current implementation status**: this app exposes the runtime provider id `gemini` (catalog id, transport id, registry id all `gemini`). The research file id `gemini-api` is only this document's workstream label — it is not a runtime id in the codebase.
- **Full package preservation status**: current code does **not** preserve the full provider package. On success the call sites return only normalized `{ text, usage }`; on failure they throw normalized errors (`PROVIDER_ERROR` / validation-status errors) with at most the first 500 chars of the body inlined into `Error.message`. Response headers, raw body string, parsed full JSON, all candidates beyond index 0, `responseId`, `promptFeedback`, `safetyRatings`, `citationMetadata`, per-modality usage breakdowns, and full structured error bodies (`error.code`, `error.status`, `error.details[]`) are all discarded.
