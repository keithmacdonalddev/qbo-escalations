...
- **Provider path type:** Spawned local CLI subprocess — `child_process.spawn('codex', [...], { shell: true })` — invoking the OpenAI Codex CLI binary on the host PATH. Auth is via the host's existing Codex CLI login/subscription state (e.g. ChatGPT login or Codex login); the application code does **not** pass an API key to the subprocess and does not set `OPENAI_API_KEY` for it. The explicit env mutation on model-work spawns is clearing `CLAUDECODE`, a guard borrowed from the Claude CLI integration to avoid nested-session signaling. Whatever auth state `codex` finds locally is what it uses.
...
- **Full package preservation status today:** **Implemented for the three model-work spawns** when `ENABLE_PROVIDER_CALL_PACKAGE_CAPTURE=true`: `codex.chat`, `codex.parseEscalation`, and `codex.transcribeImage` now preserve stdin, args, stdout text, ordered stdout JSONL events, malformed stdout lines, stdout/stderr chunks, stderr text, exit/signal facts, timeout/abort facts, timing, model, reasoning effort, and source/call-site context in `ProviderCallPackage.cli`. Operational probes (`codex.warmUp`, `codex --version` checks) are still treated as availability/startup probes, not model-work provider packages.
- **Streaming?** The CLI streams. The Node process consumes the stream and exposes it to upstream callers via `onChunk` / `onThinkingChunk` / `onDone` callbacks. The chat-leg's downstream consumer treats this as a real stream; the image-parser leg buffers it. The harness now also records the ordered stdout event stream for captured model-work calls without changing caller-visible behavior.
- **Harness boundary:** The Codex/OpenAI CLI capture layer records the subprocess package and accepts caller-supplied context. It does not decide what an image-parser prompt means, and it does not parse the model response into escalation fields. If a caller such as `image-parser.js` labels a call as `operation: "image-parse"`, that label is caller-owned metadata stored on the package for tracing. The image-parser Codex path force-captures the CLI package, receives a `providerPackageId`, then reads the saved `ProviderCallPackage.cli.stdout.jsonlEvents` from MongoDB to extract parser text. The legacy `codex.parseEscalation` helper now defaults its package operation to `parse-escalation` and can also accept caller-supplied capture context; it should not be treated as the generic OpenAI CLI harness boundary.
