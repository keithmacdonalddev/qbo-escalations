...
| Split heavy client routes. | Non-chat routes, chat presentation, and dev-only diagnostics now load as lazy chunks. Chat state remains owned by `App`, so the stream can stay mounted after the chat chunk loads. |
| Guard startup bundle size. | Added `npm run test:client-bundle`, which builds the client and fails if the initial JS, initial CSS, or largest JS chunk grows past the operating limits. Current observed output: initial JS 314.5 KiB, initial CSS 408.1 KiB, largest JS 314.5 KiB. |
