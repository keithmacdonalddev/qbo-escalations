...
Calm composition is not an empty container. On an identity-first cover, the component must remain the visual subject: reject any specimen frame whose vacant area overwhelms the specimen, makes the control look like a placeholder, or forces the next useful chapter below the viewport without adding meaning. Create polish through proportion, optical balance, type, and purposeful whitespace; do not solve density by adding facts, and do not solve cognitive load by preserving a large box after its content has been removed.
