...
- the `INVEST` lane reports whether safe connection/snapshot test states are available, authorization is stored locally, renewal is required, secure storage is locked, revocation is pending, or Questrade is not connected. It also says only whether saved portfolio data is available. This local status check never synchronizes, never contacts Questrade, and never prints a snapshot reference, time, account identity, position count, symbol, balance, token, destination, or raw provider response. “Authorization stored” deliberately does not claim that the broker is reachable; live verification is deferred until the user requests it.
