...
Every provider attempt records a safe manifest by default: provider/model identity, prompt and payload hashes, timing, usage, outcome, errors, capture purpose, and reasoning summary metrics when the provider exposes reasoning. Ordinary product traffic does not retain unrestricted request/response bodies or reasoning text. Parser, triage, and KB readback use a `provider-result-handoff-v1` record containing only bounded final assistant text, allowlisted scalar token counts, source provenance, byte length, and SHA-256; the legacy raw/parsed response and CLI event fields are omitted. Full payload and reasoning capture requires an explicit allowlisted `diagnostic` or `evaluation` purpose. `forceCapture` guarantees a manifest exists but does not authorize raw traffic retention.
...
MongoDB TTL removes the package document at its configured expiry. External payload files are reconciled separately by a bounded janitor that runs opportunistically during provider activity: it removes sidecars for expired documents and old orphan directories while retaining live documents and fresh in-flight directories. Recorder failures remove the exact package sidecar directory they may have created. Packages with sidecars record `storage.externalRetention.state = managed`, `cleanupImplemented = true`, and `orphanRisk = false`.
