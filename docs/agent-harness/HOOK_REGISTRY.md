...
| --- | --- | --- | --- | --- |
| `.claude/hooks/pm-rules.sh` | Claude `UserPromptSubmit` | Repeats critical behavior, communication, design-impact classification, Apple-research method, and rendered UI/UX acceptance rules | Appends timestamp only to `.claude/logs/pm-rules.log` | Prints no private prompt content; a hook failure does not replace root rules |
