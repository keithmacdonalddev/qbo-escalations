...
## STEP 0 — Read the QBO App's Live Design Tokens FIRST (do this before writing anything)

Do NOT trust any QBO color, font, or spacing value quoted later in this prompt — the app's theme changes over time, so treat every QBO value below as a placeholder, not fact. Before you research the brand, extract the app's **current** values directly from the codebase:

1. Open `client/src/App.css` and read the active `:root` tokens — capture the real values for `--accent` (and any accent-hover/subtle), `--bg` / background surfaces, `--text-base` (base font size), `--ink` / text colors, `--success`, `--warning`, `--danger`, `--line` / borders, and the sidebar background.
2. Open `client/src/design-system.css` and `client/src/design-system-v2.css` for the live typography, spacing scale, radius scale, and motion/easing tokens.
3. Open `client/src/hooks/useTheme.js` (and `client/src/themes/`) to see which themes exist and which is the **default/active** one — the active theme is the baseline you compare against, not a retired theme.

Wherever a table or note in this prompt shows a QBO value (e.g. an accent hex, a base font size, a background color), **replace it with the value you actually read in Step 0** and note "(read live from `<file>` on `<date>`)". If a token named below no longer exists, say so and use the closest current equivalent. Every QBO-side claim in your report must trace to a token you read in this step, not to the numbers printed in this prompt.
...
Fill the QBO App column with the values you read in Step 0 (do NOT copy the example values below — they may be from a retired theme):

| Concept | {BRAND_NAME} | QBO App (read live in Step 0) | Analysis |
|---------|-------------|-------------------------------|----------|
| Background | | `--bg` (light/dark) | |
| Text Primary | | `--ink` (light/dark) | |
| Accent | | `--accent` | |
| Success | | `--success` | |
| Warning | | `--warning` | |
| Danger | | `--danger` | |
| Info | | (token or "uses accent") | |
| Sidebar | | sidebar bg token | |

State the QBO app's **current** design identity (theme name, color temperature — warm vs cool, light-first vs dark-first) based on what you read in Step 0, then evaluate whether {BRAND_NAME}'s color temperature would work for 8+ hour support shifts.
...
Note: Read the QBO app's current font families and `--text-base` size from `design-system-v2.css` / `App.css` (Step 0) and report those — do not assume a specific px value here.
...
Note: Read the QBO app's actual spacing scale (`--sp-*` tokens) from Step 0 and report the real values.
...
Note: Read the QBO app's actual border-radius tokens from Step 0 and report the real values.
...
Note: Read the QBO app's actual motion tokens (durations, `--ease-*` curves) from `design-system.css` / `design-system-v2.css` in Step 0 and report the real values; confirm whether prefers-reduced-motion is respected.
...
**Current design identity:** Determine this in Step 0 — do not assume. The app ships multiple theme variants and the active/default theme changes over time. Read the active theme's name, color temperature (warm vs cool), and light-first vs dark-first stance from `client/src/App.css` and `client/src/hooks/useTheme.js`, and report what you find. (Historically the app used a warm "Warm Authority / Obsidian Ember" identity with an ember-amber accent; that may no longer be the default — verify rather than repeat it.)

Other identity elements to confirm live (these change less often, but check):
- Multi-layer shadows / inset highlights, gradient backgrounds on buttons and cards
- Number of theme variants available
- Category badge color system and provider identity colors

**Design tokens already in place (read the live values in Step 0):**
- CSS custom properties in :root with dark mode overrides
- Spacing scale (`--sp-*` tokens)
- Typography scale with tracking and weight compensation
- Motion durations + spring physics, reduced-motion support
- Elevation shadows (xs through 2xl, ambient variants)
