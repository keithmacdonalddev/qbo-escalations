...
For feedback/reporting specifically, keep one stable modal with two real modes: **New report** and **My reports**. In New report, the report-type chooser is the only initial decision. Each type control uses one small italic intent prompt above one centered action label; the added height must remain restrained. Selecting a type reveals the matching form inside the same fixed frame. Put field guidance in small italic text beside its label instead of spending a second row on helper copy. Screenshot capture comes before optional contact, uses one compact `Add a screenshot` row plus a short value statement, and keeps the same small `Optional` qualifier. Name and email remain visible as two compact, equal-height fields with italic `Optional` qualifiers and no explanatory paragraph below them. Required technical metadata is collected silently and disclosed through a centered, italic, low-emphasis link below the form actions; it is not presented as a checkbox or a large explanatory card. My reports is functional follow-up, not placeholder navigation: keep its receipt list, public-safe status/conversation, reply, and fixed/not-fixed validation compact inside the same frame, and never expose owner-only notes or controls.
...
| Feedback and problem reporting | `client/src/components/reporting/UserReportDialog.jsx` and `.css` | Fixed-frame new-report and My Reports flow, progressive type selection, compact optional contact/screenshot controls, and public-safe follow-up |
