...
The following reduction pass failed in the opposite direction. It removed decision-helping copy while preserving the same tall geometry, replaced meaning with `Optional` pills, and forced equal heights by bottom-aligning sparse content. The result was empty holes and less guidance in the same amount of space. Subtraction is not a goal by itself; value density is the goal.

Future UI work must apply these corrections before styling:

1. **Start with the task, not with adding or deleting.** Identify the required decision, required input, primary action, and the guidance that helps someone choose correctly. Remove only what does not support that set.
2. **Charge every visible line against the space budget without deleting decision support.** Supporting text stays visible when it resolves a likely uncertainty, prevents a meaningful mistake, or helps the user decide whether to use a control. Rare safety or policy detail belongs in a tooltip, disclosure, or linked notice.
...
5. **Consolidate before cardifying.** Closely related optional controls share one organized surface with a divider. Align their headings, guidance, and controls naturally; never manufacture equality by stretching sparse content to the bottom of a tall track.
...
7. **Respond to density criticism with better value density, not an extreme.** Remove low-value material, retain copy that changes a decision, and remeasure the actual content. Do not leave the old height behind as an empty shell.
