...
## UI correction record: verbosity is not polish

The July 24 feedback-modal iterations still failed the product standard after their first visual improvements. They treated explanatory copy, headings, badges, icons, and separate containers as if each were free. The result was more polished styling wrapped around unnecessary reading, excessive vertical rhythm, redundant messages, and uneven peer surfaces. That is cognitive load, not a premium experience.

Future UI work must apply these corrections before styling:

1. **Start by deleting.** Identify the required decision, required input, and primary action. Remove everything that does not change one of those three things.
2. **Charge every visible line against the space budget.** Supporting text stays visible only when it resolves a likely uncertainty, prevents a meaningful mistake, or changes the next action. Rare safety or policy detail belongs in a tooltip, disclosure, or linked notice.
3. **Do not narrate obvious controls.** Name and email do not need a paragraph explaining contact. Screenshot controls do not need a sentence explaining what screenshots do.
4. **Do not add a heading for content the fields already explain.** A title and detail field do not need a separate “Share the details” introduction or a “Required” badge when the workflow and validation already establish that requirement.
5. **Consolidate before cardifying.** Closely related optional controls share one organized surface with a divider. Peer sections must have equal outer geometry; uneven sibling containers are a release defect.
6. **Preserve accepted product copy.** Do not rewrite labels or prompts while solving layout unless the product owner explicitly requests a copy change.
7. **Respond to density criticism with subtraction.** Do not restyle the same information, add another hierarchy layer, or explain the design. Remove low-value material and remeasure the result.
8. **Prove the space/value result from screenshots.** Review focused crops at desktop and mobile sizes. For every visible line and container, ask what the user loses if it disappears. If the answer is “nothing,” remove it before calling the UI polished.

For feedback/reporting specifically, the modal is one new-report workflow. Open with one collaborative header line and a compact, expressive chooser—not a full-height empty form frame. The chooser does not need a separate visible `Type` label. Preserve the established user-facing choices (`Found a bug?` / `Report a Problem`, `Have an idea?` / `Request a Feature`, and `Want to chat?` / `Submit Feedback`) unless the product owner explicitly requests copy changes. Give each path a recognizable icon and a deliberate hover, focus, active, and selected state. On selection, expand into the writing frame and compress the chooser so the form becomes the priority; keep that expanded frame stable when the user switches report paths. Begin immediately with the required title and explanation fields. Keep short guidance on the same line as its label; move the privacy warning into an accessible tooltip beside the relevant field. Put screenshot evidence and optional contact in one divided support surface with equal peer geometry and no explanatory paragraphs for obvious controls. Qualify each optional area once. Anchor submission in a calm footer that pairs the human-review/data-use message with the cancel and send actions. Required technical metadata is collected silently; it is not presented as a checkbox or a large explanatory card. Do not show placeholder history tabs or disclosures for features that are not part of the current task.
