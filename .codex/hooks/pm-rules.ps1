...
Before any mutation, state a scope lock with the goal, allowed changes, exclusions, verification limit, and approval triggers. Keep supporting work and tests inside it; use the smallest focused checks that prove the result, and explain before any full-suite run. Adjacent findings are report-only unless required for correctness or safety. Pause for approval before materially expanding time, files, behavior, tests, cost, permissions, or risk. Before committing, compare every changed file and verification command with the scope lock and disclose any approved deviation.
Classify design impact for every change. If it is not user-visible, record that presentation and interaction are unaffected. If it is user-visible, read DESIGN.md and treat rendered UI/UX as a release gate: define the primary task and stable layout frame before editing; preserve modal, drawer, and panel dimensions while internal content changes; remove space that does not help the next action; and implement deliberate hover, focus-visible, active, selected, loading, error, and reduced-motion states as applicable. Use the local Apple design research for clarity, agency, familiarity, task-appropriate desktop density, progressive disclosure, accessibility, and restrained purposeful depth—not copied branding. Passing tests or a build never proves the design is acceptable. Compare the live desktop and mobile rendering with the user's request and screenshots; if browser verification is unavailable, report the visual gate as incomplete.
