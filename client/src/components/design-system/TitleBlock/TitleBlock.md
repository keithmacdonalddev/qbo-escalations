...
The approved direction was independent by design. The legacy TitleBlock implementation, its usages, and its documentation specimen were not inspected while the new direction was designed because the user explicitly requested a fresh result without that component's influence. After approval, the legacy production folder and its documentation-only consumers were inspected solely to perform a safe replacement. They did not alter the approved anatomy or API. The released component incorporates the requested fluid scale, clarified theme-variable behavior, and the later `25ch` width decision.

| Current evidence | Design implication | Decision | Strongest rejected alternative |
| --- | --- | --- | --- |
| The user requested “TitleBlock” and explicitly prohibited inspecting the existing component's functionality. | The new direction had to stand on first principles and current global design evidence rather than preserving accidental legacy anatomy. | Use a text-led, independently defined primitive and document the old implementation, consumers, and specimen as uninspected. | Reviewing the old component first would reduce migration risk, but it would violate the requested method and bias the new direction toward inherited choices. |
...
| `client/src/App.css:142-165` defines the original typography and spacing scales used by Slate. | Spacing can remain token-connected, but production title sizes must reproduce the approved measurements rather than assume the original type tokens remain effective. | Keep `--sp-1`, `--sp-2`, and `--sp-3` for the approved gaps while owning the exact 18px, 28px, and fluid title sizes inside TitleBlock. | Reusing the global title-size tokens would look tidy in source but would not reproduce the approved component under the effective cascade. |
| `client/src/overhaul.css:335-338` forces global heading color, tracking, and 600 weight, while `client/src/overhaul.css:7030-7046` redefines `--text-lg` to 16px and `--text-2xl` to 23px. | A normal component rule using the original tokens would render smaller and lose the approved scale-specific tracking. The user subsequently selected 600 as the preferred title weight. | Use collision-resistant component selectors, exact local type measurements, a normal 600 weight declaration, and narrowly scoped `!important` declarations only for color and tracking where required to beat the legacy global heading override. | Changing the site-wide density tokens or removing the global override during this component release could unexpectedly restyle unrelated screens and exceeds the approved adoption scope. |
...
The heading is the only required visible node. When `description` is absent, empty, or whitespace-only, the paragraph and its top margin are both omitted; there is no invisible spacer. Description markup and spacing disappear together. The component contains no icon column, accent bar, eyebrow row, action rail, badge slot, metadata row, or background layer.
...
Page scale uses a component-owned 28px size with a 1.16 line height and slightly negative letter spacing for a compact, confident heading. Section scale uses a component-owned 18px size with a 1.24 line height. Fluid uses the exact requested `font-size: clamp(34px, 4.2vw, 50px)`, a tighter 1.08 line height, and `-0.035em` tracking. This means it stays at 34px below roughly 810px, grows with viewport width through the middle range, and stops at 50px above roughly 1190px.

Descriptions use exact governed sizes of 14px at page scale, 13px at section scale, and 15px at fluid scale. The internal gap uses `--sp-2` (6px) for page, `--sp-1` (4px) for section, and `--sp-3` (8px) for fluid. The 34px and 50px bounds are approved component measurements; they are not global tokens and do not authorize changing `App.css`.

The description measure is capped near 68 characters at page scale and 62 at section scale. Fluid instead uses an 86ch cap: at its 15px description size that resolves to about 695px in the approved typeface, visually aligning it with the fluid heading's roughly 694px maximum rather than forcing an early wrap near 485px. The component itself does not impose a fixed outer width. The fluid heading uses a 25-character measure on broad layouts so concise identity-first titles remain controlled at 50px; that measure releases at narrow width. Both title and description use natural wrapping plus an emergency `overflow-wrap: anywhere` safeguard for unbroken identifiers. No ellipsis is allowed because truncating a title can hide the user's location.

Borders, radii, shadows, and elevation are absent from the component. The prototype's specimen frames are explicitly presentation scaffolding and are not part of V2. This distinction is important: a contained panel may compose the primitive, but TitleBlock does not own that layer or gain card styling through a `surface` prop.

## Interaction and state journey

TitleBlock is static and deliberately non-interactive. Resting is its only intrinsic state. Hover, focus-visible, pressed, selected, open, disabled, waiting, success, failure, recovery, and focus restoration are inapplicable because the primitive exposes no action, control, or changing status. It never enters the tab order and never attaches click or keyboard handlers.
...
Color is not required to understand the component. Primary and secondary ink establish hierarchy alongside size, weight, position, and wording. The production component is checked on the documentation Slate surface at desktop and exactly 390px, with forced-color behavior and full-text wrapping preserved. There is no keyboard interaction, focus style, target size, announcement, or reduced-motion alternative to implement because there is no control or motion. Responsive wrapping, full text preservation, and the absence of fixed height protect zoom, localization, and large-text use.
...
The production API keeps document semantics, visual scale, optional information, and integration identity separate:
...
- Unsupported scales warn and fall back to page. This preserves the canonical visual contract.
...
- Whitespace-only heading IDs are ignored. Other invalid empty values warn instead of emitting an unusable `id` attribute.
...
- Arbitrary visual styling is prohibited because `style`, `className`, raw color, and surface props are excluded from the public contract. Styling and interaction escape hatches are ignored with development warnings.
...
TitleBlock names one area and may explain its purpose. A plain semantic heading is preferable for local groups that do not need the shared title-description rhythm. A PageHeader is a parent composition that may arrange TitleBlock with breadcrumbs, actions, or page-level layout. A SectionHeader may later compose the section scale with a dedicated action relationship if repeated evidence justifies it.
...
- [x] Heading level and visual scale remain separate API axes in the released contract.
...
- [x] Optional `headingId` labels a surrounding region without creating a landmark.
...
- [x] Empty required content and invalid runtime values have deterministic production safeguards.
- [x] `style`, `className`, arbitrary children, polymorphism, actions, and raw visual props are excluded from the production API.
- [x] Approved prototype evidence preserves the accepted anatomy; the user-approved `25ch` source revision is explicitly recorded as an intentional bitmap difference.
- [x] The explanation validator reports `STRUCTURE PASS`.
- [x] Production implementation began only after the user explicitly approved this direction.
- [x] Production desktop and exact-390 rendering are reconciled against the approved direction before `available` status.
...
TitleBlock is a static, typography-led heading primitive that names the current work area and optionally explains its purpose without absorbing decoration, status, navigation, or action behavior.
