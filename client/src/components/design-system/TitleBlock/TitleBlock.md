...
| `client/src/overhaul.css:335-338` forces global heading color, tracking, and 600 weight, while `client/src/overhaul.css:7030-7046` redefines `--text-lg` to 16px and `--text-2xl` to 23px. | A normal component rule using the original tokens would render smaller and lose the approved 660 weight and scale-specific tracking. | Use collision-resistant component selectors, exact local type measurements, and narrowly scoped `!important` declarations only where required to beat the legacy global heading override. | Changing the site-wide density tokens or removing the global override during this component release could unexpectedly restyle unrelated screens and exceeds the approved adoption scope. |
